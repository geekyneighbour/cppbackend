...
int main(int argc, char* argv[]) {
    try {
        ...
        auto handler = std::make_shared<http_handler::RecordsRequestHandler>(args->www_root, strand, game);
        
        std::shared_ptr<http_handler::DatabaseObserver> db_observer;
        const char* db_url = std::getenv("GAME_DB_URL");
        if (db_url && strlen(db_url) > 0) {
            db_observer = std::make_shared<http_handler::DatabaseObserver>(db_url);
    ...
    game.AddObserver(db_observer.get());
        ...
        auto token_remover = std::make_shared<http_handler::TokenRemoverObserver>(*handler);

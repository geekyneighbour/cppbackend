...
// Используем Point2D из geom.h
using PointDouble = geom::Point2D;
...
// FoundObject для совместимости с прекодом
struct FoundObject {
    using Id = util::Tagged<uint32_t, FoundObject>;
    Id id{0u};
    size_t type{0u};
    
    auto operator<=>(const FoundObject&) const = default;
};

// BagItem для совместимости
struct BagItem {
    size_t id;
    size_t type;
...
public:
    ...
    double GetMaxX() const noexcept {
        return std::max(static_cast<double>(start_.x), static_cast<double>(end_.x));
    }
    
    double GetMinY() const noexcept {
        return std::min(static_cast<double>(start_.y), static_cast<double>(end_.y));
    }
...
public:
    ...
    const Id& GetId() const noexcept { return id_; }
    uint64_t GetIdValue() const { return *id_; }
...
public:
    ...
    size_t GetNextLootId() const { return next_loot_id_; }

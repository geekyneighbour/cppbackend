// request_handler.h - ПОЛНАЯ ВЕРСИЯ
#pragma once
...
public:
    ...
    RequestHandler(fs::path root, Strand strand, model::Game& game,
                   const json_loader::LootTypesStorage& loot_types,
    ...
    void OnTick(std::chrono::milliseconds delta) {
        if (!auto_tick_mode_) return;
        
        // Обновляем состояние игры
        game_.UpdateAllSessions(static_cast<double>(delta.count()) / 1000.0);
        
        // Генерируем трофеи для всех сессий
        for (const auto& map_ptr : game_.GetMaps()) {
            auto& session = game_.FindOrCreateSession(map_ptr.get());
            const auto* map = session.GetMap();
            if (!map) continue;
            
            size_t loot_count = session.GetLootCount();
            size_t looter_count = session.GetDogs().size();
            ...
            if (to_generate > 0) {
                session.GenerateLoot(to_generate);
            }
        }
    }
...
private:
    ...
    json_loader::LootTypesStorage loot_types_;
    ...
    http::response<http::string_body> HandleApiRequest(const Req& req) {
        ...
        if (path.starts_with(MAPS2) && path.size() > MAPS_PREFIX_LENGTH) {
            ...
            // Добавляем lootTypes из хранилища
            auto it = loot_types_.map_loot_types.find(map->GetId());
            if (it != loot_types_.map_loot_types.end()) {
                result["lootTypes"] = it->second;
            }

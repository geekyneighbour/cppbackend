...
void AddOffices(const boost::json::array& offices_array, model::Map& map) {
    for (const auto& o_val : offices_array) {
        ...
        int offsetX = static_cast<int>(o_obj.at("offsetX").as_int64());
        int offsetY = static_cast<int>(o_obj.at("offsetY").as_int64());

        map.AddOffice(model::Office(model::Office::Id{id}, model::Point{x, y}, model::Offset{offsetX, offsetY}));
    }
}
...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    if (!file.is_open()) {
        throw std::runtime_error("Failed to open JSON config file: " + json_path.string());
    }

    std::stringstream buffer;
    buffer << file.rdbuf();
    
    auto root = boost::json::parse(buffer.str());
    if (!root.is_object()) return game;

    const auto& obj = root.as_object();
    
    LoadGlobalSettings(obj);

    if (auto config_loot = obj.if_contains("lootGeneratorConfig")) {
        if (config_loot->is_object()) {
            const auto& loot_obj = config_loot->as_object();
            double period = loot_obj.at("period").as_double();
            double probability = loot_obj.at("probability").as_double();
    ...
    for (const auto& map_val : obj.at("maps").as_array()) {
        ...
        if (auto loot_types = map_obj.if_contains("lootTypes")) {
            if (loot_types->is_array()) {
                ...
                ExtraDataStorage::GetInstance().SaveLootTypes(id, loot_types->as_array());
            }
        }
